...
AldroneTeleop::AldroneTeleop() {
    ...
    sub_joy = nh_.subscribe<sensor_msgs::Joy>(
        "joy", 10, &AldroneTeleop::JoyCallBack, this);
...
void AldroneTeleop::JoyCallBack(const sensor_msgs::Joy::ConstPtr& joy) {
    if (!got_first_joy_msg) {
        ROS_INFO("Found joystick with %zu buttons and %zu axes",
...
int main(int argc, char* argv[]) {
    ...
    ros::Rate spinRate(aldrone_teleop.kSpinRate);
